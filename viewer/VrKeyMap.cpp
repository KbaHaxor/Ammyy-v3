...
const UINT32 VoidKeyCode = XK_VoidSymbol;

// Define the keymap structure
typedef struct VrKeymapping_struct {
	UINT wincode;
	UINT Xcode;
} VrKeymapping;
...
static const VrKeymapping keymap[] = {
...
KeyActionSpec VrKeyMap::PCtoX(UINT virtkey, DWORD keyData) 
{ 
	UINT numkeys = 0;
    ...
    //_log2.Print(8,VTCLOG(" keyData %04x "), keyData);
    
    if (extended) { 
        //_log2.Print(8, VTCLOG(" (extended) "));
        switch (virtkey) {
			case VK_MENU :	 virtkey = VK_RMENU;		break;
			case VK_CONTROL: virtkey = VK_RCONTROL;		break;
			case VK_RETURN:	 virtkey = VK_KEYPAD_ENTER; break;
        }
    }

    // We try looking it up in our table
    UINT mapsize = sizeof(keymap) / sizeof(VrKeymapping);
    ...
    if (numkeys != 0) {
		...
		if ( (key == XK_Alt_L || key == XK_Alt_R) && GetKeyState(VK_SCROLL) ) {
			kas.keycodes[numkeys-1] = (key == XK_Alt_L) ? XK_Meta_L : XK_Meta_R;
		}
        //_log2.Print(8, VTCLOG("keymap gives %u (%x) "), key, key);

    } else {
        // not found in table
        //_log2.Print(8, VTCLOG("not in special keymap, "));
		...
		int ret = ::ToAsciiEx(virtkey, 0, keystate, (WORD *) buf, 0, m_hrl);
        ...
        if (  ((keystate[VK_MENU] & 0x80) != 0) && ((keystate[VK_CONTROL] & 0x80) != 0) ) {
           ...
           if  ( (ret >= 1) && 
               ...
               if (kas.releaseModifiers == 0)
				...
				//_log2.Print(8, VTCLOG("Ctrl-Alt pressed: ToAscii (without modifiers) returns %d byte(s): "), ret);
                for (int i = 0; i < ret; i++) {
                   kas.keycodes[numkeys++] = *(buf+i);
                   //_log2.Print(8, "%02x (%c) ", *(buf+i) , *(buf+i));
                }
                //_log2.Print(8,"\n");
           } 
        ...
        if (numkeys == 0) {
            ...
            if (ret >= 1) {
                //_log2.Print(8,VTCLOG("ToAscii (without ctrl) returns %d byte(s): "), ret);
                for (int i = 0; i < ret; i++) {
                   kas.keycodes[numkeys++] = *(buf+i);
                  //_log2.Print(8, VTCLOG("%02x (%c) "), *(buf+i) , *(buf+i));
                }
            }
        }
